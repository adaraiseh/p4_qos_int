...
function do_p4c {
    ...
    # check if tx_utilization needs to be added 
    if ! grep -q "@alias(\"queueing_metadata.tx_utilization\")" "$input_file"; then
        # If it doesn't exist, insert it before the closing brace
        sed -i '/struct standard_metadata_t {/,/}/{ 
            /}/i\
        // port tx utilization\
        @alias("queueing_metadata.tx_utilization")\
        bit<32> tx_utilization;
        }' "$input_file"
    fi
